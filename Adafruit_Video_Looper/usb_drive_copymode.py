    ...
    def copy_files(self, paths):
        ...
        for path in paths:
            ...
            if self._config.has_option('playlist', 'path'):
                playlist_path = '{0}/{1}'.format(path.rstrip('/'), self._config.get('playlist', 'path'))
                if playlist_path != "":
                    if not os.path.isabs(playlist_path) and os.path.isfile(playlist_path):
                        self.clear_screen()
                        self.draw_info_text("Copying playlist file...")
                        time.sleep(2)
                        self.copy_with_progress(playlist_path, '{0}/{1}'.format(self._target_path.rstrip('/'), os.path.basename(playlist_path)))
